...
// 将reg_addr赋值给指令寄存器，将reg_val赋值给数据寄存器
...
// 从图形存储器GRAM中读取一个坐标为(x, y)的像素的颜色数据
/* 读取指定地址的GRAM */
...
void lcd_clear(unsigned short Color) // 定义函数，参数值为颜色
{
	unsigned int count; // 定义一个无符号整数，用来计数

	write_cmd(0x002a); // 表示要设置水平方向的显示区域
	write_data(0); // 表示显示区域的起始点的水平坐标的高八位，即0   
	write_data(0); // 表示显示区域的起始点的水平坐标的低八位，即0
	write_data(HDP>>8); // 表示显示区域的终止点的水平坐标的高八位，即HDP的高八位   
	write_data(HDP&0x00ff); // 表示显示区域的终止点的水平坐标的低八位，即HDP的低八位
	
  write_cmd(0x002b); // 设置垂直方向的显示区域
	write_data(0); // 显示区域的起始点的垂直坐标的高8位，即0
	write_data(0); // 显示区域的起始点的垂直坐标的低8位，即0
	write_data(VDP>>8); // 表示显示区域的终止点的垂直坐标的高八位，即VDP的高八位
	write_data(VDP&0x00ff); // 表示显示区域的终止点的垂直坐标的高八位，即VDP的低八位
	
	write_cmd(0x002c); // 表示要开始写入的颜色
	write_cmd(0x002c); // 重复发送指令，保证正确性


	for(count=0;count<130560;count++){ // 循环130560次，即显示区域的像素点总数
		write_data(Color); // 表示像素点的颜色值，即参数Color
	}
}
...
{
	//uint16_t old_color = lcd_read_gram(xsta, ysta);
	//if(old_color == color)
	//{
	//	return;
	//}
...
void Draw_Line(uint16_t x0, uint16_t y0, uint16_t x, uint16_t y, uint16_t color, int quadrant) {
	switch (quadrant) {
		case 1:
			LCD_DrawLine(x0 + x, y0 + y, x0 + x, y0, color);
			LCD_DrawLine(x0 + y, y0 + x, x0, y0 + x, color);
			break;
		case 2:
			LCD_DrawLine(x0 - x, y0 + y, x0 - x, y0, color);
			LCD_DrawLine(x0 - y, y0 + x, x0, y0 + x, color);
...
void Fill_Circle(uint16_t x0, uint16_t y0, uint8_t r, uint16_t color) {
	...
	while (a <= b) {
		...
		Draw_Line(x0, y0, a, b, color, 1); // ????
		Draw_Line(x0, y0, a, b, color, 2); // ????
		Draw_Line(x0, y0, a, b, color, 3); // ????
		Draw_Line(x0, y0, a, b, color, 4); // ????
